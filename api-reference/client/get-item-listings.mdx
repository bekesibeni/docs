...
```http
GET https://api.assetpay.gg/client/market/item?itemId=AK-47%20%7C%20Redline%20(Field-Tested)&delivery=normal&page=1
...
Each listing represents a specific item instance with its own float value, stickers, and price. Use the `id`, `offer.price`, and `offer.reference` when initiating a withdrawal.
